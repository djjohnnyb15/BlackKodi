...
def Archive_File(sourcefile, destfile):
    ...
    for_progress = []
    ITEM =[]
    dp.create("[COLOR=white]Shadow Builds[/COLOR]","Archiving...",'', 'Please Wait')
    for base, dirs, files in os.walk(sourcefile):
        for file in files:
            ITEM.append(file)
    N_ITEM =len(ITEM)
...
def Check_Download_Path():
    ...
    if not os.path.exists(zip):
        dialog.ok('[COLOR=white]Shadow Builds[/COLOR]','The download location you have stored does not exist .\nPlease update the addon settings and try again.','','')        
...
def Fix_Special(url):
    dp.create("[COLOR=white]Shadow Builds[/COLOR]","Renaming paths...",'', 'Please Wait')
...
def Install_Art(path):
    ...
    try:
        ...
        time.sleep(1)
#        Read_Zip(artpack)
        dp.create("[COLOR=white]Shadow Builds[/COLOR]","Checking ",'', 'Please Wait')
...
def Restore_Backup_XML(name,url,description):
    if 'Backup' in name:
        ...
        if 'guisettings.xml' in description:
            ...
            f.write(TO_READ)
            f.close()  
    dialog.ok("[COLOR=white]Shadow Builds[/COLOR]", "", 'All Done !','')
#---------------------------------------------------------------------------------------------------
#---------------------------------------------------------------------------------------------------
...
def Restore_Local_Community():
    ...
    if exitfunction == 1:
        ...
        dialog.ok('[COLOR=white]Shadow Builds[/COLOR]','Step 1 complete. Now please change the skin to','the one this build was designed for. Once done come back','to this addon and restore the guisettings_fix.zip')        
        xbmc.executebuiltin("ActivateWindow(appearancesettings)")
#---------------------------------------------------------------------------------------------------
...
def Restore_Zip_File(url):
    ...
    if 'Backup' in name:
        Delete_Packages() 
        dp.create("[COLOR=white]Shadow Builds[/COLOR]","Backing Up",'', 'Please Wait')
        ...
        zipobj.close()
        dp.close()
        dialog.ok("[COLOR=white]Shadow Builds[/COLOR]", "You Are Now Backed Up", '','')   
    else:
        dp.create("[COLOR=white]Shadow Builds[/COLOR]","Checking ",'', 'Please Wait')
        dp.update(0,"", "Extracting Zip Please Wait")
        ...
        if 'Backup' in name:
            ...
            dialog.ok("[COLOR=white]Shadow Builds[/COLOR]", "You Are Now Restored", '','')        
#---------------------------------------------------------------------------------------------------
