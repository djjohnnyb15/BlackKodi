...
def Wipe_Kodi():
    ...
    if choice == 1:
        if skin!= "skin.confluence":
            dialog.ok('[COLOR=white]Shadow Builds[/COLOR]','Please switch to the default Confluence skin','before performing a wipe.','')
            ...
            choice = xbmcgui.Dialog().yesno("Remove Shadow Builds?", 'Do you also want to remove the Shadow Builds', 'add-on and have a complete fresh start or would you', 'prefer to keep this on your system?', yeslabel='Remove',nolabel='Keep')
            if choice == 0:
                cache.Remove_Textures()
                ...
                dp.create("[[COLOR=white]Shadow Builds[/COLOR][/B]","Checking ",'', 'Please Wait')
...
#Builds Section
def BuildMenu():
    link = OPEN_URL('http://shadowcrew.info/twins/Community_Builds/NewMenu.txt').replace('\n','').replace('\r','')
...
def BrettusMenu():
    link = OPEN_URL('https://archive.org/download/BrettusBuilds/Brettus_Builds.txt').replace('\n','').replace('\r','')
    match = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(link)
    for name,url,iconimage,fanart,description in match:
        addDir(name,url,'wizard',iconimage,fanart,description)
    setView('movies', 'MAIN')
...
def wizard(name,url,description):
    ...
    dp = xbmcgui.DialogProgress()
    dp.create("Shadow Builds ","Downloading ",'', 'Please Wait')
...
elif mode == 'brettusbuilds'	  : BrettusMenu()
